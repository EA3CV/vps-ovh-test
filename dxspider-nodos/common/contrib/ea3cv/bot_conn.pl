#!/usr/bin/perl

#
# Bot that sends the connection/disconnection of nodes
# via Telegram
#
# Usage: bot_conn.pl server
# eg     bot_conn.pl EA3CV-2
#
# Created by Kin EA3CV
# ea3cv@cronux.net
# v1.2
# 20200615
#

use strict;
use warnings;
use 5.10.1;
use Time::Piece;
#use Time::Seconds;

$ENV{TZ} = "UTC";

my $server = $ARGV[0];

my $time = Time::Piece->new;
my $m = $time->mon();
$m = sprintf("%02d", $m);
my $y = $time->year("");

my $log = "/spider/local_data/log/$y/$m.dat";

# Verificar si existe fichero de log
if ( !-f $log) {
	open (LOG,">$log"); 
	close LOG;
}

open my $data, "-|", "/usr/bin/tail", "-n1", "-f", $log or die "could not start tail on $log: $!";

while (my $line = <$data>) {
	next unless ($line =~ /connected/);

	# 1590336672^DXCommand^EC3BH connected from 213.4.177.78
	# 1590336759^DXCommand^EC3BH disconnected
	# 1590337177^DXProt^IW9EDP-6 Disconnected
	# 1590337500^DXProt^IW9EDP-6 connected from 151.54.254.233

	my @campos = split("\\^", $line);
	$campos[2] =~ m/(\w+||\w+\-\d+)\s(\w+)/;

	if (($campos[1] eq "DXProt") && ($2 eq "connected")) { 
		telegram($server,"<-----", "$1 Conectado");
        } elsif (($campos[1] eq "DXProt") && ($2 eq "Disconnected")) {
                telegram($server,"--//--", "$1 Desconect");
  	}
}

close $data;


sub telegram {
        my $srv = shift;
        my $type = shift;
        my $state = shift;

	my $token = "1079220543:AAHX9uEYzvsIKVQoaEPVc6iARoXH2GaNKwU";
	my $id = "1089814914";
	my $url = "https://api.telegram.org/bot$token/sendMessage";
	`curl -s -X POST $url -d chat_id=$id -d text="$srv $type $state\n"`;
}
