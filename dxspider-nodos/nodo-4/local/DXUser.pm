...
our $backend = $main::db_backend || 'file';
...
sub init
{
		...
		if ($backend eq 'mysql' or $backend eq 'sqlite') {
			require DXUser_SQL;

			*get           = \&DXUser_SQL::get;
			*put           = \&DXUser_SQL::put;
			*new           = \&DXUser_SQL::new;
			*alloc         = \&DXUser_SQL::alloc;
			*del           = \&DXUser_SQL::del;
			*close         = \&DXUser_SQL::close;
			*get_all_calls = \&DXUser_SQL::get_all_calls;
			*sync          = \&DXUser_SQL::sync;
			*export        = \&DXUser_SQL::export;
			*recover       = \&DXUser_SQL::recover;

			return DXUser_SQL::init($mode);
		}
