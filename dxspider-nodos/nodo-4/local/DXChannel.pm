...
#
## Kin
# - Inserción del comentario por columnas (inicio fijo)
# - Llamada a Local::predictor que devuelve SOLO el comentario enriquecido (sin padding)
# - Saltar VHF/UHF (> 30 MHz en kHz)
...
sub send                        # this is always later and always data
{
    ...
    # Columna (0-based) donde empieza SIEMPRE el comentario para este usuario/skin.
    # Ajusta si tu skin usa otra (tu regla mostraba 39).
    my $COMMENT_COL = 39;
    ...
    foreach my $l (@_) {
        for (ref $l ? @$l : $l) {
            ...
            for (@lines) {
                ...
                # Debe terminar en HHMMZ; si no, enviar tal cual
                my ($ts) = ($line =~ /(\d{4}Z)$/);
                unless (defined $ts) {
                    $conn->send_later("D$call|$line");
                    dbg("-> D $call $line") if isdbg('chan');
                    next;
                }

                # Extraer spotter, freq (kHz) y dx; si falla, enviar tal cual
                my ($spotter, $freq, $dx) = ($line =~ /^DX de\s+(\S+):\s+(\d+(?:\.\d+)?)\s+(\S+)/);
                unless (defined $spotter && defined $freq && defined $dx) {
                    $conn->send_later("D$call|$line");
                    dbg("-> D $call $line") if isdbg('chan');
                    next;
                }

                # No tocar VHF/UHF (>30 MHz en kHz)
                if ($freq > 30000) {
                    $conn->send_later("D$call|$line");
                    dbg("-> D $call $line") if isdbg('chan');
                    next;
                ...
                my $len    = length($line);
                my $ts_pos = rindex($line, $ts);           # índice donde empieza HHMMZ

                # Validaciones de rango
                if ($COMMENT_COL >= $ts_pos || $COMMENT_COL >= $len) {
                    $conn->send_later("D$call|$line");
                    dbg("-> D $call $line") if isdbg('chan');
                    next;
                }

                # Partes por posición fija
                my $head          = substr($line, 0, $COMMENT_COL);                 # antes del comentario
                my $comment_block = substr($line, $COMMENT_COL, $ts_pos - $COMMENT_COL); # campo comentario exacto
                my $tail          = substr($line, $ts_pos);                         # desde HHMMZ al final

                # Para la API: comentario sin padding derecho
                (my $api_comment = $comment_block) =~ s/\s+$//;

                dbg("DXChannel TX user=".$self->{call}
                    ." spotter=$spotter freq=$freq dx=$dx ts=$ts"
                    ." comment='$api_comment' width=".(length($comment_block))) if isdbg('predictor');

                # Llamada a Local.pm → devuelve SOLO el comentario enriquecido (sin padding)
                my $new_comment = $api_comment; # fallback
                if (defined &Local::predictor) {
                    eval {
                        $new_comment = Local::predictor($self, $line, $freq, $dx, $ts, $api_comment, $spotter);
                    };
                    dbg("DXChannel eval error: $@") if $@ && isdbg('predictor');
                }

                # Ajustar al ancho EXACTO del campo comentario, dejando 1 espacio antes de la hora
                my $width_total = length($comment_block);
                my $width       = $width_total > 0 ? $width_total - 1 : 0;  # reservar 1 espacio separador

                $new_comment //= $api_comment;
                my $fixed = substr($new_comment, 0, $width);
                $fixed .= ' ' x ($width - length($fixed));
                $fixed .= ' ';  # el espacio reservado antes de HHMMZ

                # Recomponer por columnas fijas
                my $out = $head . $fixed . $tail;

                dbg("DXChannel final line='$out'") if isdbg('predictor');

                $conn->send_later("D$call|$out");
                dbg("-> D $call $out") if isdbg('chan');
            }
        }
    }
