...
#               if ($pcno == 34 || $pcno == 36) { last SWITCH; } # remote commands (incoming)
#               if ($pcno == 35) { last SWITCH; }                # remote command replies
#               if ($pcno == 37) { last SWITCH; }
#               if ($pcno == 38) { last SWITCH; }                # node connected list from neighbour
#               if ($pcno == 39) { last SWITCH; }                # incoming disconnect
#               if ($pcno == 41) { last SWITCH; }                # user info
...
#               if ($pcno == 50) { last SWITCH; }                # keep alive/user list
#               if ($pcno == 51) { last SWITCH; }                # ping req/ans
        }
        return 0;
}
...
#
#       # Si quieres modificar SOLO el comentario antes de difundir:
#       # my $user = { call => $main::mycall };  # o el destinatario, si se llama por usuario
#       # my $line = '';                         # no usado por predictor()
#       # my $new_comment = predictor($user, $line, $freq, $spotted, $d, $text, $spotter);
#       # $_[3] = $new_comment;  # sustituye el comentario que se va a difundir
#       #
...
# ======================================================
# ============  PREDICTOR (añadido)  ===================
# Devuelve SOLO el comentario nuevo (sin padding ni truncar).
sub predictor {
    my ($user, $line, $freq, $dx, $ts, $comment, $spotter) = @_;
    my $target = $user->{call};

    dbg("Local.pm RX user=$target freq=$freq dx=$dx ts=$ts comment='$comment' spotter=$spotter") if isdbg('predictor');

    # kHz; ignorar > 30 MHz
    return $comment if !defined($freq) || $freq > 30000;

    my $ua  = LWP::UserAgent->new(timeout => 0.5);
    my $url = "http://localhost:8000/predict";

    my $payload = {
        callsign_user    => $target,
        callsign_spotter => $spotter,
        callsign_dx      => $dx,
        frequency        => 0.0 + $freq,   # kHz; el backend convierte si procede
        timestamp        => format_ts($ts),
        comment          => $comment // ""
    };

    my $json = encode_json($payload);
    dbg("Local.pm API TX JSON: $json") if isdbg('predictor');

    my $res = $ua->post($url, 'Content-Type' => 'application/json', Content => $json);

    if ($res->is_success) {
        my $data = decode_json($res->decoded_content);
        my $new_comment = $data->{new_comment} // $comment;
        dbg("Local.pm API RX comment='$new_comment'") if isdbg('predictor');
        return $new_comment;  # sin padding; DXChannel ajusta al ancho
    } else {
        dbg("Local.pm API ERROR: " . $res->status_line) if isdbg('predictor');
        return $comment;
    }
}
...
sub format_ts {
    ...
    if ($ts =~ /^(\d{2})(\d{2})Z$/) {
        my ($h, $m) = ($1, $2);
        my @lt = gmtime();
        return sprintf("%04d-%02d-%02dT%02d:%02d:00Z",
            $lt[5]+1900, $lt[4]+1, $lt[3], $h, $m);
    }
    return $ts;
}
# ======================================================
