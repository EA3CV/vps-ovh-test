...
push @out, " ";
push @out, "   Callsign   Type          Started             Connection time";
push @out, "   --------   -----------   -----------------   ------------------";
...
#    "  %2d m %3d d %3d h %3d m",
    ...
    $delta->[2], $delta->[3], $delta->[4]
        ...
        if ($type eq "NODE") {
                push @out, sprintf "   %-10s $type   $sort   $t $fecha", $call;
...
push @out, " ";
push @out, "   Nodes: $all_nodes  Load: $total1 %  CPU: $fields[1] %  Mem: $total2 %  Uptime: $uptime";
push @out, " ";
